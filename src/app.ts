...
import { NotConnectedError, SyncBusyError, syncNow, syncWorkouts } from './sync';
...
type Context = {
  ...
  path: string;
  /**
   * Somewhere to put work that should outlive the response. A route that
   * changes the plan uses it to sync to the athlete's watch platform without
   * the write waiting on a third party — see `syncWorkouts`.
   */
...
const createWorkout: Handler<AuthedContext> = async ({ request, url, env, user, background }) => {
...
const replaceWorkout: Handler<AuthedContext, WorkoutRoute> = async ({
  request,
  url,
  env,
  user,
...
const deleteWorkout: Handler<AuthedContext, WorkoutRoute> = async ({ env, user, params, background }) => {
  const date = parseDate(params.date, 'date');
...
const runTool: Handler<AuthedContext, '/api/tools/:name([a-z_]+)'> = async ({
  request,
  url,
  env,
  user,
 ...
 */
const syncEverything: Handler<AuthedContext> = async ({ request, url, env, user }) => {
...
const routes = new Router<Context>({
  ...
  .post('/api/sync', withUser(syncEverything))

  // Connecting is per platform, because every consent flow differs; syncing
  // is not, which is why only the routes above are provider-neutral.
