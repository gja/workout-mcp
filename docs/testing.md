...
Google, Apple and intervals.icu are stood in for by an auxiliary Worker
(`test/upstream-provider.js`) that Miniflare routes all outbound traffic to, so
the real `arctic` path runs — Apple's signed client secret included — without
